...
-- Find the ID and app name for all apps that were last updated
-- on August 01, 2018.
...
-- Find the app that has the most reviews with a rating greater than 
-- equal to 4.8.
SELECT * FROM analytics 
    WHERE rating >= 4.8 
    ORDER BY reviews DESC 
...
-- Find the name, price, and rating of the most expensive app with a 
-- rating that’s less than 3.
...
-- Find the names of all apps that are rated less than 3 with at least 
-- 10000 reviews.
SELECT app_name FROM analytics 
    WHERE rating < 3 
        AND reviews >= 10000;

-- Find the top 10 most-reviewed apps that cost between 10 cents and 
-- a dollar.
SELECT * FROM analytics 
